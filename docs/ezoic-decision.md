...
> 동결 중에 만들어 둔 것: **#110 상세 시트 자리(103) 제거** — feat 브랜치에 준비 완료, 10/01
> 이후 `/release`. 근거는 아래 2026-09-17 항목.
...
- 🔴 **배포는 10/01 이후.** 아래 7번 "측정 동결"(9/15~10/01)이 `AdSlot` 배치 변경을 금지한다.
  9/15·9/22 판독이 이 결정의 마지막 깨끗한 근거이므로, feat 브랜치에 만들어 두고 동결이 끝난
  뒤 `/release`한다. 배포 후에는 display 묶음이 아니라 **111 단독** viewability·RPM을 이 표와
...
2. **Ezoic 자동 사이드레일(116/117)이 우리 레일(107/108)과 나란히 돈다** — 넷을 합쳐
   ...
   vs 우리 $0.38~0.42, 현재는 우리 쪽)을 남긴다. 이것도 10/01 이후.
