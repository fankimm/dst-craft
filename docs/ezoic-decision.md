...
- `docs/ezoic-ticket-bid-timeout.md` — Ezoic 지원 티켓 초안 (입찰 타임아웃 하향, 미제출 — 9/22 판독 뒤 제출)
...
### 7. 측정 동결: 9/15 ~ 9/22 (2026-09-21 단축 — 원래 10/01)
...
> **2026-09-21 — 동결 끝을 10/01에서 9/22로 당겼다.** 동결이 지키는 건 9/15·9/22 두 판독뿐이고
> 10/01은 근거 없는 여유분이었다. 9/22 자정이 지나면 9/16~9/22 구간이 통째로 깨끗하게 남으므로
> **9/23부터 광고 변경 가능.** 판독 자체는 Ezoic 리포트가 이틀 늦게 확정되니 9/24~25에 한다 —
> display 묶음이 아니라 **placeholder 111 단독**으로 읽을 것 (자리별 리포트, 아래 2026-09-17 항목).
> 9/23 이후에도 광고 변경은 **한 번에 하나씩** 내보낸다 — 겹치면 어느 변경이 숫자를 움직였는지 못 읽는다.
>
> 동결 중에 만들어 둔 것: **#110 상세 시트 자리(103) 제거** — 근거는 아래 2026-09-17 항목.
>
> **2026-09-21 (같은 날 재결정) — 동결을 오늘로 끝내고 #110을 9/21에 배포했다.** 9/19까지의
> Revenue Daily가 수정 전후로 평평해(맨 아래 "수익 추세 판독" 절) 9/22 판독이 결론을 바꿀 여지가
> 없다고 보고, 사용자가 즉시 릴리즈를 지시했다. 따라서 **placeholder 111 단독 판독의 깨끗한
> 구간은 9/16~9/20**이다 (9/21부터는 #110 배포 후 구간).

## 2026-09-17 — placeholder별 실측: 상세 시트 자리(103)는 뺀다 (#110)

지금까지 이 문서의 표는 **Ad Format**(display / side rails / …) 단위라 우리 자리 111과 103이
"display" 한 줄에 묶여 있었다. Ezoic 분석의 **Ad Placeholder** 리포트는 자리 번호별로 갈라
준다 (`analytics.ezoic.com` → Yield → Ad Placeholder. 화면이 크롬 확장에 안 잡히면 pubdash
탭에서 `ezservice.ezoic.com/analytics/v1/reports/getData?report=placeholder&chartName=p_table`
을 같은 세션으로 fetch — DataTables식 `columns[i][data]` 파라미터를 전부 넣어야 한다).

### 9/2 ~ 9/15 (수정 후, 합계 13,771 impr / $6.13 / RPM $0.44 / viewability 82.86%)
| id | 이름 | Impr. | Revenue | 비중 | RPM | CTR | **Viewability** |
|---|---|---|---|---|---|---|---|
| 111 | mid_content (상단 띠) | 4,365 | $1.16 | 18.9% | $0.26 | 0.55% | 78.37% |
| 100 | Anchor Ad (Ezoic 자동) | 2,010 | **$1.35** | 22.0% | $0.67 | 0.25% | 86.72% |
| 15 | standalone video (Ezoic 자동) | 1,315 | $0.97 | 15.8% | $0.73 | 2.05% | 100.00% |
| 107 | sidebar_floating_1 (왼쪽 레일) | 1,240 | $0.52 | 8.5% | $0.42 | 0.32% | 92.98% |
| 108 | sidebar_floating_2 (오른쪽 레일) | 1,205 | $0.46 | 7.5% | $0.38 | 0.25% | 92.37% |
| **103** | **bottom_of_page (상세 시트)** | 1,164 | **$0.29** | **4.7%** | **$0.25** | 0.52% | **51.46%** |
| 6 | vignette (Ezoic 자동) | 945 | $0.91 | 14.8% | $0.96 | 2.01% | 76.61% |
| 116 | Side Rail Left (Ezoic 자동) | 801 | $0.26 | 4.2% | $0.33 | 0.12% | 88.51% |
| 117 | Side Rail Right (Ezoic 자동) | 726 | $0.22 | 3.6% | $0.30 | 0.00% | 87.19% |

### 8/8 ~ 9/1 (수정 전, 합계 13,321 impr / $6.48 / RPM $0.49 / viewability 76.46%)
| id | 이름 | Impr. | Revenue | 비중 | RPM | **Viewability** |
|---|---|---|---|---|---|---|
| 111 | mid_content | 4,352 | $1.44 | 22.2% | $0.33 | 71.23% |
| 6 | vignette | 2,597 | $2.09 | 32.3% | $0.80 | 78.09% |
| **103** | **bottom_of_page** | 1,737 | **$0.65** | **10.0%** | $0.38 | **51.01%** |
| 107 | sidebar_floating_1 | 1,341 | $0.77 | 11.9% | $0.57 | 89.78% |
| 108 | sidebar_floating_2 | 1,290 | $0.66 | 10.2% | $0.51 | 90.78% |
| 116 | Side Rail Left | 926 | $0.43 | 6.6% | $0.46 | 89.09% |
| 117 | Side Rail Right | 870 | $0.36 | 5.6% | $0.42 | 88.62% |
| 15 | standalone video | 177 | $0.08 | 1.2% | $0.46 | 100.00% |
| 100 | Anchor Ad | 3 | $0.00 | — | — | 66.67% |

(리포트 `lastDate`가 9/15라 최신 이틀은 빠진다. 8/8~9/1에 1~13 노출짜리 잔여 번호
110·112·113·115·84가 있는데 #75 초기에 잠깐 쓰던 번호들의 흔적이다.)

### 읽히는 것

- **103은 두 기간 모두 viewability 51%로 전 자리 최저다.** 다른 자리는 전부 76~100%.
  노출의 절반이 한 번도 보이지 않은 채 끝난다 — 사용자가 보러 온 정보(스탯·재료)가 광고
  위에서 끝나 스크롤할 동기가 없고, 짧은 아이템은 광고가 시트를 늘리는 유일한 원인이었다.
  고정 `SupportPill`이 소재 위로 겹치는 정책 리스크도 있었다(스크린샷 확인).
- **수익 비중은 4.7%(수정 후) / 10%(수정 전).** 시트를 닫으면 사용자가 다시 111을 보게
  되므로 이 금액 일부는 다른 자리로 옮겨간다. 시트를 열고 닫을 때마다 나가던 4자리
  재배치(~2초 공백)와 그걸 막던 #96 안정화 타이머의 존재 이유이기도 했다.
- **→ 자리를 뺀다 (#110).** `DetailPanel`의 `sheet` 자리와 `AdSlot`의 103 매핑·claim/linger
  타이머 제거. 회귀 스크립트는 이제 시트 열기·유지·닫기 단계에서 **배치 0**을 요구한다.
- 🔴 **배포는 9/23.** 위 7번 "측정 동결"이 `AdSlot` 배치 변경을 금지한다. 9/15·9/22 판독이
  이 결정의 마지막 깨끗한 근거이므로 feat 브랜치에 만들어 두고 9/22가 지난 뒤 `/release`한다
  (처음엔 10/01로 잡았다가 9/21에 단축 — 7번 참조). 배포 후에는 display 묶음이 아니라 **111 단독** viewability·RPM을 이 표와
  비교할 것 — 103이 빠지면 display 합계 viewability는 산술적으로 오르지만 그건 개선이 아니다.

### 겸사겸사 드러난 두 가지 (별건, 결정 필요)

1. **Anchor(100)가 9/2 이후 살아났다** — 8/8~9/1 노출 3회 → 9/2~9/15 2,010회·$1.35로
   전체 1위. 우리 코드에 100번은 없으니(`todo.md` "anchor RPM $0.00" 항목) Ezoic이 자동
   앵커를 켠 것이다. 9/2 배포와 시기가 겹치지만 인과는 미확인. 9/8 항목의 "수익 상승은 다른
   요인" 절반이 이것일 가능성이 크다 — **ePMV 전후 비교를 읽을 때 이 자리를 빼고 볼 것.**
2. **Ezoic 자동 사이드레일(116/117)이 우리 레일(107/108)과 나란히 돈다** — 넷을 합쳐
   레일 수익 $1.46(23.8%). 같은 화면에 레일이 4개 뜨는지, 아니면 화면 폭에 따라 갈리는지
   실브라우저(1500px 이상)에서 확인 필요. 겹친다면 `disableLeftSideRail` 계열 `config()`
   로 자동 쪽을 끄거나, 우리 107/108을 빼고 자동에 맡기는 것 중 RPM 높은 쪽(자동 $0.30~0.33
   vs 우리 $0.38~0.42, 현재는 우리 쪽)을 남긴다. 이것도 동결 이후, #110 배포와 시기를 나눠서.

## 2026-09-21 — 지연 의심 7항목 재점검 + 도달률 3차 판독 (#124)

"광고가 4~10초 늦게 뜬다 — 우리 연동이 잘못된 것 아니냐"는 물음이 다시 나와, 흔히 지목되는
7항목을 **현재 코드와 prod**로 다시 대조했다. 결론은 8/27·8/31과 같다: **코드로 고칠 수 있는
1~6번은 병목이 아니고, 원인은 7번(Ezoic·Google 파이프라인)이다.** 같은 질문이 또 나오면
이 표부터 볼 것 — 다시 파지 않는다.

| # | 의심 항목 | 판정 | 근거 (2026-09-21 재확인) |
|---|---|---|---|
| 1 | CMP 스크립트 순서 | 정상 | prod HTML에서 `gatekeeperconsent` @1.9KB → `sa.min.js` @2.1KB. `AD_SCRIPT_SRCS` + `s.async=false`(#79). 직렬화 페널티 중앙값 0ms |
| 2 | `next/script` strategy | 해당 없음 | Ezoic은 `next/script`를 안 쓴다 — `</head>` 직전 인라인 부트스트랩 + `ReactDOM.preload()`. `afterInteractive`는 GSI 하나뿐 |
| 3 | placeholder가 SSR HTML에 없음 | 병목 아님 | 홈·`/item` 모두 SSR에 0개(클라이언트 렌더)인 건 사실. 그러나 스탠드얼론은 초기 스캔이 아니라 `showAds(...ids)` 명시 호출이고, 우리 push가 Ezoic 큐 드레인보다 680~777ms 먼저 도착해 기다린다 |
| 4 | `showAds` 타이밍·중복 | 정상 | `AdSlot` 배치 큐가 같은 틱을 한 번의 `showAds`로 묶음. `FLUSH_DEBOUNCE_MS=250`은 위 슬랙에 숨는다 (낮추면 #75 재발) |
| 5 | Cloudflare Rocket Loader 등 | 영향 없음 | prod HTML에 `rocket-loader`/`rocketscript` 0건, `cf-cache-status: DYNAMIC`, CMP 2개는 `data-cfasync=false` |
| 6 | TTFB (맥미니 + Tunnel) | 병목 아님 | 5회 측정 208~375ms, HTML 136KB 완료 0.26~0.50초 |
| 7 | Ezoic 측 (입찰 타임아웃 등) | **원인** | `window load` 게이트 + 랜덤 입찰 타임아웃 500~2000ms + 하류 바닥 2.5~3.5초 ("왜 느린가" 절) |

### 실사용자 첫 광고 도착 시각 — 누적 (n=3,957)

`GET /api/stats` 의 `adFillMs` 누적. 8/31 판독(n=675)과 **분포 모양이 그대로**다 — 표본이
6배로 늘어도 최빈 구간·비율이 안 움직였다. #91~#97 프론트 경량화가 도착 시각을 당기지
못했다는 뜻이고, 하류 바닥 가설과 맞는다.

| 구간 | 수 | 비율 | (8/31 비율) |
|---|---:|---:|---:|
| 2-4s | 103 | 2.6% | 3.6% |
| 4-6s | 1,132 | 28.6% | 24.0% |
| 6-8s | 1,524 | **38.5%** | 38.5% |
| 8-12s | 894 | 22.6% | 24.4% |
| 12-20s | 272 | 6.9% | 7.9% |
| 20s+ | 32 | 0.8% | 1.6% |

중앙값 약 **7.0초** (6-8s 구간 선형 보간). headless 실측(약 5초)보다 실사용자가 2초 느리다.
`todo.md` 3-2의 "미해결: headless 절대값이 실사용자와 다를 수 있다"는 이것으로 닫는다 —
**다르다, 실사용자가 더 느리다.**

### 도달률 3차 판독 — 9/14~9/20 (완결된 7일)

`GET /api/stats?days=14`. 산식은 8/31·9/8과 동일 `(early + noscript) ÷ 전체`.

| 날짜 | early | noscript | filled | nofill | blocked | 총 | 판정값 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 09-14 | 337 | 75 | 144 | 73 | 0 | 629 | 65.5% |
| 09-15 | 347 | 52 | 143 | 63 | 0 | 605 | 66.0% |
| 09-16 | 597 | 56 | 150 | 80 | 0 | 883 | **74.0%** |
| 09-17 | 665 | 36 | 108 | 72 | 1 | 882 | **79.5%** |
| 09-18 | 343 | 35 | 133 | 51 | 3 | 565 | 66.9% |
| 09-19 | 489 | 64 | 145 | 56 | 1 | 755 | 73.2% |
| 09-20 | 391 | 108 | 200 | 69 | 0 | 768 | 65.0% |
| **7일 합산** | 3,169 | 426 | 1,023 | 464 | 5 | **5,087** | **70.7%** |

- **판정값 63.1%(8/31) → 66.1%(9/8) → 70.7%.** 임계 40%에서 더 멀어졌다. 9/6~9/7의 58.8%는
  추세가 아니라 저점이었다
- 광고를 실제로 본 세션 **20.1%** (8/31 23.6%). 판정된 세션 안의 채움률은 68.8%(8/31 63.6%)로
  오히려 올랐다 — **no-fill이 아니라 `early`가 손실의 전부**라는 8/31 결론이 더 선명해졌다
- 9/16·9/17에 `early`만 600건대로 튄다(`filled`는 평소 수준). 원인 미확인 — 봇·단발 유입이면
  판정값을 부풀린다. 이 이틀을 빼도 5일 합산은 **67.5%** 라 결론은 같다
- ⚠️ 가역성 경고(9/8 절)는 그대로 유효하다. 이 숫자만으로 이탈을 결정하지 말 것

### 남은 조치 — 동결(9/15~9/22) 이후에만

(동결은 9/21에 10/01 → 9/22로 단축했다가, 같은 날 아래 수익 추세 판독을 보고 **9/21로 종료**했다 — 위 7절 참조. 아래 "9/22 판독 전에는 보내지 않는다"는 제약도 함께 풀렸다.)

코드 쪽 회수 후보는 8/27에 전부 0ms로 확인됐다. 남은 건 Ezoic에 요청하는 것 하나다:

- **Ezoic 지원 티켓** — 초안은 `docs/ezoic-ticket-bid-timeout.md`. 입찰 타임아웃 상한 하향 +
  `insticator`·`yieldmo` 검토 요청. **9/22 판독 전에는 보내지 않는다** — Ezoic이 설정을 바꾸면
  그 자체가 측정 구간 오염이다 (위 "7. 측정 동결")

### 수익 추세 판독 — 8/19~9/19 Revenue Daily (9/22 판독을 이것으로 갈음)

사용자가 공유한 `analytics.ezoic.com` Revenue Daily 그래프(8/19~9/19, 데이터 확정 상한 9/19)를
**눈대중으로 읽은 값**이다 — CSV가 아니라 ±10% 오차가 있다. 창은 도메인 승인(8/20) 직후부터
데이터 상한까지 전 구간으로 잡았다.

| 구간 | 일평균 수익 | 일평균 ePMV (일별 단순평균) |
|---|---:|---:|
| 8/19~8/31 (광고 유실 수정 **전**) | 약 $0.38 | 약 $1.0 |
| 9/1~9/7 (수정 직후) | 약 $0.37 | 약 $1.2 |
| 9/8~9/19 | 약 $0.43 | 약 $1.3 |

- ePMV는 $1.0 → $1.3으로 올랐지만 **상승분의 대부분이 하루짜리 스파이크**(9/5 $2.10, 9/12 약 $1.85,
  9/15 약 $2.33)다. 바닥은 $0.7~1.0 그대로
- 금액으로는 일 +$0.05, **연 $18**. 연 환산 $140 → $155
- 일 변동폭(9/15 $0.66 → 9/16 $0.22)이 구간 평균 차이보다 훨씬 크다 — 이 규모에서는 어떤
  설정 변경의 효과도 노이즈에 묻힌다
- 포맷별 display RPM($0.34) 판독은 하지 않았다. 전체가 이 규모면 그 값이 결론을 못 바꾼다

**측정은 여기서 끝낸다.** #93~#97 수정도, 프론트 경량화도, 도달률 계측 3회도 같은 답이다:
코드로 움직일 수 있는 건 다 움직였고 지표는 그대로다. 남은 건 10/15 결정 하나 —
**연 $150 안팎을 위해 첫 광고 7초·`early` 60%·1년 약정을 유지할 것인가.**
