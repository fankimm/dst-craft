...
- **후속 (2026-08-18)**: Identity는 실제로 `Verified`로 전환됨. 남은 건 Address 하나이며 여전히 요청 전. 셋업이 안 끝난 진짜 원인은 별개로 `Ad Manager Domain Status: In Progress`(8/17 제출, 최대 14일)였다 — **미완료 항목이 여러 개 보일 때 그중 무엇이 실제 병목인지부터 가릴 것**
...
- **교훈 4**: `<link rel="preload">`를 JSX로 `<head>`에 쓰면 React가 hoist한 사본과 원본이 **둘 다** HTML에 나가 중복된다(`<body>`로 옮겨도 동일). 이번엔 실측상 요청 시작 시각이 안 변해서 그냥 뺐다 — **최적화는 넣기 전에 재고, 이득이 없으면 넣지 않는다**
