...
- **문제**: `cookpot-ingredients.ts`에서 `forgetmelots`에 `dryable: true`가 걸려 `forgetmelots_dried`가 자동 생성되는데, "Dried-only ingredients" 블록에 같은 ID가 수동으로 또 등록돼 있었음. 요리솥 재료 고르기의 전체/즐겨찾기 탭에 말린 건망초가 두 번 표시되고 리스트 key도 중복 (사용자 피드백: "말린건망초 즐겨찾기하면 진짜 건망초마냥 무한번식해요")
