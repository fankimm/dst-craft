...
### 위키 서술을 "소스 기준" 문서에 확인 없이 옮겨 적음 + 상수만 읽고 판정 (2026-09-17, #117 → #118)
- **문제**: 농사 조사 문서(`docs/farming-research.md`)의 "소스 기준 메커니즘" 절에 위키 문장 두 개("작물은 낮밤 없이 물을 마신다", "물풍선 +25")를 그대로 적었다. 코드는 `growable:IsGrowing()`일 때만 마시고(밤엔 0), 물풍선은 +20이다. 또 공략의 오류를 판정할 때 `tuning.lua` 상수 하나만 보고 "소스는 25"라고 단정했다
- **원인**: 위키를 먼저 읽고 그 틀로 소스를 "확인"하는 순서라, 위키가 말한 항목 중 상수가 눈에 띄는 것만 대조하고 나머지는 맞다고 넘어갔다. 상수가 **어떤 함수에서 어떻게 쓰이는지**는 보지 않았다
- **교훈**: 위키 수치는 코드 조건과 종류가 다를 수 있다. ① 코드는 조건인데 위키는 현재 최소 단위를 적는 경우(요리 `tags.meat` = 고기 존재 여부, 위키 "고기 0.5") ② 위키가 맞는 실효값인데 상수와 달라 보이는 경우(거대 용과 보관 16일 = 6일 × 4 ÷ 바닥 부패 1.5) ③ 위키가 그냥 틀린 경우(얼음 20/물풍선 25는 서로 바뀐 값). **상수 → 그 상수를 쓰는 함수 → 그 함수를 부르는 곳**까지 따라가야 셋을 구분할 수 있다
- **규칙**: "소스 기준"이라고 적는 문장은 직접 읽은 코드만. 위키에서 온 서술은 "위키 서술(미검증)"으로 표시한다. 조사 문서에는 검증 수준(경로 추적 / 상수만 / 미검증) 표를 둔다
- **덤**: 경로를 끝까지 따라가면 공략에 없는 사실이 나온다 — 일반 씨앗에서 자란 작물은 `no_oversized`라 거대 작물이 될 수 없다 (`farm_plants.lua` `ReplaceWithPlant`)
...
### 결정 문서에 이미 있는 실사용자 분포를 "새로 확인된 것"이라고 보고했다 (2026-09-21, #124)
- **문제**: "광고 4~10초 지연" 의심 7항목을 점검하면서 `/api/stats`의 `adFillMs`(중앙값 약 7초)·`early` 59%를 **"8/27 미해결이 오늘 풀렸다, 새 발견"** 으로 사용자에게 보고했다. 실제로는 `docs/ezoic-decision.md`가 8/31에 같은 분포(n=675, 최빈 6-8s 38.5%)를, 9/8에 일별 도달률 표를 이미 기록하고 "손실의 정체는 `early`"라는 결론까지 내려 둔 상태였다. 새로운 건 표본이 6배가 된 것과 9/14~9/20 판독뿐이었다
- **원인**: 문서를 **키워드 grep(`지연|delay|showAds…`)으로만** 훑었다. 1,000줄짜리 문서에서 매치된 줄 몇 개와 `todo.md` 3-2만 읽고 "기록된 건 여기까지"라고 단정했다. `todo.md`의 "미해결" 표시가 안 닫혀 있던 것도 오판을 거들었다 — 결론은 결정 문서에만 적히고 `todo.md`는 갱신이 안 됐다
- **교훈**:
  - CLAUDE.md의 "광고 건은 이 문서를 먼저 확인"은 **목차(`grep '^## '`)부터 읽으라는 뜻**이다. 날짜별 절 제목만 봐도 "실사용자 계측이 말하는 진짜 문제"가 있다는 걸 10초 만에 안다. 키워드 grep은 내가 떠올린 단어로 쓰인 줄만 보여준다
  - "새로 확인"이라고 말하기 전에 **그 숫자의 키 이름(`adFillMs`, `early`)으로 문서를 한 번 더 grep** 할 것. 이미 있으면 "기존 기록과 같다/달라졌다"로 보고한다
  - 같은 사실이 두 문서에 걸쳐 있으면 한쪽의 "미해결"은 거짓 신호가 된다. 결론을 적을 때 그 질문을 열어 둔 쪽도 같이 닫을 것 (이번에 `todo.md` 3-2를 닫았다)
- **영향**: 결론·권고는 안 바뀜. 문서에 기록하려고 기존 절을 읽다가 바로 발견해 사용자에게 정정했고, 중복 기록 대신 "8/31 대비 분포 불변"이라는 비교로 바꿔 적었다
...
### 페이지뷰 3개(HAR)로 수익 유닛을 끄자고 제안했다 — 대시보드는 정반대였다 (2026-08-31, #92)
- **문제**: HAR 3회에서 Interstitial이 `br1` 500→0까지 내려가고도 전부 빈 응답(no-fill)인 걸 보고 "3.2초를 쓰고 광고는 안 나온다"며 `disableInterstitial` 을 끌 후보로 올리고 beta에 켰다. **Ezoic 대시보드 최근 7일 집계는 정반대였다** — interstitial이 수익의 **29%**($0.61/$2.08)이고 **RPM $0.72로 셋 중 가장 높다**(display $0.31의 2.3배)
- **원인**: **표본이 페이지뷰 3개**였다. 광고 채움은 세션·지역·시각·재고에 따라 흔들리는 확률 변수인데, 연속된 3개 세션이 전부 no-fill인 걸 "이 유닛은 안 채워진다"로 읽었다. 게다가 3개 전부 **beta**에서, 그중 2개는 **헤드리스**로 잡은 것이라 Ezoic의 클라이언트 분류 함정(이미 오답노트에 있음)까지 겹쳤다
- **두 번째 오판**: `disableVideo` 도 "이 앱엔 비디오 컨텐츠가 없다"는 이유로 켰는데, 대시보드에서 **Ezoic Video는 "Get Started" = 미설정** 상태였다. 즉 그 옵션이 끄는 건 우리가 안 쓰는 플레이어가 아니라 **아웃스트림 비디오 광고**이고, 그건 별도 수익 항목이 없어 display 수익에 섞여 있을 수 있다(3개 포맷 합 $1.88 vs 총액 $2.08 — $0.20 미분류). **옵션이 정확히 무엇을 끄는지 확인하지 않고 이름으로 추측**했다
- **더 큰 교훈 — 자릿수를 먼저 봤어야 했다**: 대시보드 30일 수익이 **$5.55**(ePMV $0.93, 연 환산 ~$66)였다. 이 규모를 먼저 알았다면 "첫 소재를 3.2초 앞당기는" 최적화에 그만한 조사를 쏟지 않았을 것이다. **최적화 대상의 총량을 모르는 채로 비율 개선을 파고들었다**
- **교훈**:
  1. **수익이 걸린 판단은 수익 데이터로 한다.** 네트워크 관측(HAR·헤드리스)은 "무엇이 언제 요청됐나"만 답하고 "그게 얼마를 버나"는 답하지 못한다. `pubdash.ezoic.com` → Ad Format Revenue 를 **먼저** 본다
  2. **확률적 결과는 3개 표본으로 판정하지 않는다.** no-fill/fill은 베르누이 시행이다. 3/3은 채움률 60%인 유닛에서도 6.4% 확률로 나온다
  3. **설정 옵션은 이름이 아니라 대시보드의 실제 상태로 확인한다.** `disableVideo`가 무엇을 끄는지는 Ezoic Video 설정 여부를 봐야 알 수 있었다
  4. **최적화 전에 총량을 먼저 잰다.** "몇 % 개선"의 가치는 분모가 정한다. 분모($66/년)를 나중에 알면 그동안의 조사가 통째로 헛돈다
...
### 절차 문서에 없는 관례(답변 번역 백필)를 다음 세션이 몰라 병기로 우회 (2026-09-08 발생 · 2026-09-16 정리, #102 → #106)
- **문제**: 영어 피드백 `1788851925065-6uqdmo`에 Claude 답변을 달면서 `reply` 한 필드에 영문+국문을 병기했다. 보드는 `pickDisplay`가 사용자 로캘별로 원문/번역 중 하나만 보여주도록 설계돼 있고, 선례(첫 영어 피드백, 스킬트리 요청)는 영어 답변 + `replyTranslated` 국문으로 처리돼 있었는데 그 관례를 따르지 않았다. 결과적으로 모든 사용자가 두 언어를 다 봤고 번역 필드 4개는 전부 비어 있었다
- **원인**: CLAUDE.md "Feedback Replies" 절차가 id 확인 → 문구 확인 → 등록 → 확인 4단계뿐이라 번역 단계가 없었다. 번역 관례는 `translate-existing-feedback.ts` 상단 주석과 닫힌 이슈 #31/#70/#72에만 있었다 — **그 파일을 열어 본 세션만 알 수 있는 지식**이었고, #102 세션은 절차만 따라가다 두 언어 독자를 한 번에 만족시키려 병기를 택했다. 이슈 AC에도 "영문+국문"으로 적혀 있었고 사용자 확인도 받았으니 판단 실수라기보다 **절차 문서의 공백이 만든 우회**
- **부수 발견**: `PATCH /feedback`은 `reply`를 바꿔도 `reply_translated`를 지우지 않는다. 답변을 고치면 옛 번역이 stale로 남으므로 규칙에 수동 NULL 처리를 명시했다
- **해결 (#106)**: CLAUDE.md 절차에 "답변은 원문 언어로만, 병기 금지" 규칙과 "번역 백필" 단계를 추가. 해당 건은 `reply`를 영문만 남기고 국문을 `replyTranslated`로 이관, 메시지 번역도 백필
- **교훈**: 어떤 작업을 반복 가능한 절차로 CLAUDE.md에 적을 때는 **그 작업의 후속 관례까지 한 절차 안에 넣을 것**. 스크립트 주석·닫힌 이슈에만 있는 규칙은 다음 세션에겐 없는 규칙이다. 그리고 "잘 하던 게 어느 순간 안 된다"면 실행한 세션의 판단보다 **절차 문서에 그 단계가 있는지를 먼저 의심**할 것
...
### 배포 직후 실측 = 옛 번들 — 같은 세션에서 두 번 속았다 (2026-09-01, #93·#94)
- **문제**: `/beta` 배포가 `success` 로 끝난 직후 바로 실측했더니 수정 전과 **똑같은 결과**가 나왔다. `#93`에서는 "수정이 안 먹혔다"고 오진해 노드 보존 여부를 한참 파고들었고, `#94`에서도 같은 착각을 반복했다. 둘 다 실제로는 **브라우저/엣지가 아직 옛 JS 청크를 주고 있었던 것**이고, 잠시 뒤 다시 재니 정상이었다
- **놓친 신호**: `#93` 때 beta 결과가 prod와 **바이트 단위로 동일**했다. 코드가 다른데 결과가 같으면 그건 "수정이 무효"가 아니라 "같은 코드를 보고 있다"는 뜻이다
- **왜 반복했나**: GitHub Actions의 `success` 를 "배포 완료 = 사용자에게 반영됨"으로 읽었다. 실제로는 심볼릭 스왑 → 엣지 캐시 → 브라우저가 새 청크 URL을 받는 데까지 시간이 더 걸린다
- **교훈**:
  1. 배포 후 실측 전에 **새 번들을 받고 있는지부터 확인**한다. 값싼 확인법: `check-ad-slots.mjs` 처럼 우리 코드의 동작을 직접 드러내는 하네스를 먼저 돌리거나, 청크 해시/버전 문자열을 확인한다
  2. **"수정 전과 결과가 완전히 동일"하면 코드가 아니라 배포를 먼저 의심한다.** 수정이 틀렸다면 보통 *다르게* 틀리지 똑같이 나오지 않는다
  3. 이 레포의 실측 스크립트는 캐시를 안 지운다 — 새 컨텍스트를 써도 CDN 엣지는 그대로다

### `showAds`는 다른 자리를 안 건드린다고 믿었다 — 레일이 그래서 영구 사망했다 (2026-09-01, #94)
- **문제**: 상세패널을 열면 사이드 레일 광고 `#107`·`#108` 이 높이 0인 빈 div가 되고, **시트를 닫아도 돌아오지 않았다.** 그 세션 동안 레일은 죽은 채로 남는다 — 레일은 최근 7일 수익의 13%다. 탭 전환에서도 같은 일이 일어나고 있었다
- **원인**: `#75`에서 "지목하지 않은 자리까지 비워지는" 현상을 **`destroy` 전용**으로 결론내고, `flushAdQueue` 가 해제(destroy)가 낀 배치에서만 살아 있는 나머지를 함께 재요청하게 했다. 코드 주석에도 "destroy 없이 show만 — Ezoic은 다른 자리는 건드리지 않는다(#75 실측)"라고 못박혀 있었다. **그 전제가 틀렸다.** `showAds` 단독으로도 똑같이 비운다
- **왜 오래 안 잡혔나**: ① 회귀 테스트(`check-ad-slots.mjs`)가 Ezoic 목업이라 "우리가 무엇을 요청하는가"만 보고 "그 요청이 다른 자리를 죽이는가"는 볼 수 없었다 ② destroy가 낀 경로(시트 닫기)에서는 복구 로직이 돌아 정상으로 보였다 — **고장난 경로(여는 쪽)와 멀쩡한 경로(닫는 쪽)가 붙어 있어서** 닫고 나면 되살아날 것처럼 보였는데 실제로는 아니었다
- **잘못 짚은 과정도 기록해 둔다**: 처음엔 `showAds(103)`(새 자리 추가)이 범인이라고 봤다. 홈에서 `ezstandalone.showAds(111)` / `showAds(103)` 을 손으로 불러 보니 레일이 멀쩡해서 그 가설이 깨졌다. 실제 호출을 가로채(`showAds`/`destroyPlaceholders` 래핑) 보고 나서야 **상세 시트를 열 때 나가는 배치가 `showAds(111)` 하나뿐**이고 그게 레일을 죽인다는 게 보였다. **추측으로 재현하려 하지 말고 실제 호출을 로깅했어야 빨랐다**
- **교훈**:
  1. "이 API는 X만 건드린다"는 결론은 **그 API의 모든 호출 형태**로 확인해야 한다. `#75`는 `destroy`가 낀 경우만 보고 일반화했다
  2. 배치를 낼 때는 **그 시점에 보이는 자리를 전부** 넣는다. 빠뜨린 자리는 비워지고, destroy가 없으면 복구도 안 나가 영구히 죽는다
  3. 목업 회귀 테스트는 "우리가 무엇을 요청하는가"까지만 검증한다. **그 요청의 부작용은 실제 광고 위에서 봐야 한다** — `check-ad-slots.mjs` 에 "배치에서 빠진 자리" 검사를 넣어 우리 쪽 불변식으로는 잡히게 했지만, 부작용 자체의 확인은 여전히 실측 몫이다
  4. 서드파티 SDK 동작이 의심되면 **함수를 래핑해 실제 호출을 찍는다.** 밖에서 상태만 관찰하며 가설을 세우는 것보다 훨씬 빠르다

### 레포에 적힌 금지 사항의 *범위*를 확인 안 하고 넓게 해석해 "교착"이라 결론냈다 (2026-09-08, #100)

`todo.md:75`에 *"AdSense 사이트 연결 플로우 누르지 말 것 — §2와 충돌"* 이라고 적혀 있었다.
이걸 근거로 **"AdSense 승인을 받으려면 확인 태그가 필요한데 그게 금지돼 있으니 교착"** 이라고
사용자에게 보고했다. 낙하산 비유까지 써가며.

실제로는:
- 구글의 사이트 확인 방법은 **3가지**이고, 그중 **메타 태그**는 *"AdSense 코드를 넣고 싶지
  않을 때의 대안"* 으로 공식 문서에 명시돼 있다. 광고를 게재하지 않는다.
- Ezoic이 공개한 제약은 *"remove all non-Ezoic **ad codes**"* — **광고 코드**에 대한 것이다.
- 덤으로, 같은 줄이 근거로 든 *"ads.txt도 Ezoic 것만"* 은 Ezoic 자사 KB가 반박한다
  (*"add any others (like your AdSense ID)"*).
- 그리고 `검토 필요`는 거절이 아니라 **"제출된 적 없음"** 이었다. 아무도 버튼을 안 눌렀을 뿐.

**무엇을 잘못했나**: 레포 메모는 *어떤 상황에서 무엇을 하지 말라*는 좁은 지시였는데,
그걸 *그 영역 전체가 금지*로 읽었다. 그리고 그 해석을 **1차 출처로 확인하지 않은 채**
"선택지가 존재하지 않는다"는 강한 결론까지 밀고 갔다.

**교훈**: "X는 금지돼 있다"가 결론을 좌우할 때는 **금지의 정확한 범위를 원문에서 확인**한다.
특히 우리가 과거에 남긴 메모는 *그때의 맥락*에서 쓰인 것이다 — 이 메모도 MCM 설정 중
"불필요하고 위험하다"는 뜻으로 쓴 것이지, "영원히 AdSense 승인을 받지 말라"가 아니었다.

**신호**: **"그러므로 선택지가 없다"** 는 결론이 나오면 일단 멈춘다. 대개 없는 게 아니라
안 찾아본 것이다. 실제로 여기서도 막힌 게 아니라 **아무도 무료 버튼을 안 누른 것**뿐이었다.

**부수 교훈**: 이번 검증 워크플로도 반대로 틀렸다 — 공개 약관 페이지만 보고 *"Ramp-up 조항은
존재하지 않는다"* 고 단정했는데, Ezoic 대시보드에는 `dstcraft.com — Ramp-up period`가 도메인별로
버젓이 떠 있다. **공개 문서에 없다 ≠ 계약에 없다.** 계정에 귀속된 문서는 로그인해서 봐야 한다.

### 벤더의 가입 요건을 "무제한"으로 적어두고 1년 계약 판단의 전제로 썼다 (2026-09-08, #98)

`docs/ezoic-decision.md`는 선택지를 비교하면서 *"Mediavine 5만 세션, Raptive 10만,
Monumetric 1만+셋업비 — 전부 문턱이 있고 **Ezoic만 무제한이다**"* 라고 적었고, 그 위에
"AdSense가 더 나쁘면 Ezoic 복귀는 새 1년 계약" 이라는 **가역성 가정**을 세웠다. 40% 임계
규칙 전체가 이 가정 위에 있다.

실제로는 Ezoic 요건 KB가 **250,000+ monthly active users**를 요구하고, 2026-02-19 이전
게시자만 유예하며, **연동을 7일 넘게 떼면 유예가 소멸**한다고 적혀 있다. 우리는 월 UV 1.5만,
수익화 시작 2026-08-13(유예 기준일 이후)이다. 복귀가 "비싼 것"이 아니라 **불가능**일 수 있다.

**무엇을 잘못했나**: 가입할 때 사실이었던(또는 사실이라고 들었던) 조건을 그대로 문서에 박고,
**그 조건이 판단의 전제가 되는 시점에 다시 확인하지 않았다.** 벤더 정책은 조용히 바뀐다 —
이 건도 우리 가입 6개월 전에 바뀌어 있었다.

**교훈**: 계약·요금·가입 요건처럼 **벤더가 일방적으로 바꿀 수 있는 조건**을 결정의 전제로
쓸 거면, 결정 시점에 **1차 출처(벤더 공식 문서)를 다시 열어 확인**하고 확인 날짜를 함께
적는다. 특히 "무제한", "언제든 해지 가능", "페널티 없음" 같은 **되돌릴 수 있다는 서술**은
비대칭 리스크의 핵심이라 반드시 재확인 대상이다.

**부수 교훈**: 이번엔 조사 에이전트가 자기 주장 17건을 스스로 반증했고 그중 2건은
**날조된 인용문**이었다("You can disconnect anytime without penalties" — 실제 문구는
"You can remove access at any time"). 벤더 문서를 근거로 쓸 때는 **원문을 직접 열어
인용문이 실재하는지** 봐야 한다. 이 문서의 250k·7일 룰 인용은 직접 fetch해서 확인한 것이다.

### 이미지 확장자 일괄 치환이 접두사 없는 참조를 놓쳐 프로덕션에 404를 배포했다 (2026-09-01, #95)
- **문제**: `#91`에서 아이콘을 PNG → WebP로 옮기며 소스의 확장자를 정규식으로 일괄 치환했는데, **`/images/ui/{health,sanity,hunger,cooktime}.png` 4개가 404인 채로 프로덕션에 나갔다.** 요리 탭 카테고리 3개와 요리솥 재료 피커 탭 아이콘이 깨진 상태로 배포돼 있었고, 사용자가 다른 작업 중 스크린샷에서 발견하기 전까지 몰랐다
- **원인**: 치환 정규식이 `(category-icons|images/ui)/...\.png` 였다. 그런데 이 레포는 **접두사 없는 파일명을 렌더 시점에 조립**하는 곳이 있다 — `image: "ui/health.png"`(`CookingApp`), `icon: "cooktime.png"`(`IngredientPicker`, `/images/${imgDir}/` 로 조립), `console-commands.ts`. 소스 문자열만 보면 최종 경로를 알 수 없으니 정규식이 못 잡았다
- **왜 검증에서 안 걸렸나**: `#91`에서 "404 0건, naturalWidth=0인 이미지 0건"을 확인했는데 **홈과 딥링크 8개 탭만** 봤다. 깨진 아이콘은 요리 탭 **카테고리 화면**과 요리솥 **재료 피커**에 있어서 그 경로를 안 밟았다. 즉 검증 커버리지가 변경 범위보다 좁았다
- **교훈**:
  1. **파일을 옮기는 변경은 소스 치환으로 끝내지 말고 "옮긴 파일이 실제로 로드되는지"로 검증한다.** 정적 스캔은 이 레포에서 구조적으로 불가능하다(경로가 런타임에 조립된다)
  2. 검증 경로는 **변경 범위를 덮어야 한다.** 아이콘 62장을 건드렸으면 그 아이콘이 쓰이는 화면을 전부 밟아야지, 홈만 보면 안 된다
  3. `scripts/check-broken-images.mjs` 신설 — 실제로 띄워 각 탭과 하위 화면을 돌며 404와 `naturalWidth===0` 을 센다. 프로덕션에서 4건 검출됨을 음성 대조로 확인
  4. 확장자·경로를 바꾸는 코드모드는 **치환 후 "남은 불일치 0"을 파일시스템과 대조**해서 확인한다 (참조된 경로가 `public/` 에 실제로 있는지)
...
### CLS 회귀 테스트가 Ezoic 래퍼를 흉내 안 내서 prod의 36px 시프트를 통과시킴 (2026-09-09, #104)
- **문제**: 사용자가 "광고 카드 안에 실제 광고가 훨씬 작다"고 신고. prod 실측 결과 728×90 배너에 카드 높이 163px — 남는 73px 중 48px이 **Ezoic이 placeholder 안에 심는 래퍼** 몫이었다 (`span.ezoic-ad`의 자체 CSS `margin: 15px 0` 30px + 신고 줄 `.reportline` 18px). 예약 높이 100px은 배너만 계산한 값이라 광고가 도착할 때마다 **36px씩 밀리고 있었는데**, `scripts/check-ad-cls.mjs`는 계속 "띠 계열 전부 시프트 0"이었다
