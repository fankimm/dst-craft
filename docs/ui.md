...
- **가로 띠만 상시 예약(`reserve`)** — 띠는 컨텐츠 위에 있어 늦게 도착하면 목록이 밀린다(CLS, 유입 65%가 구글). 띠 계열 중 가장 높은 320×100 + Ezoic 신고 줄 18px = **118px 하나로 고정**한다(브레이크포인트로 나누지 말 것 — 데스크탑만 90px로 좁혔더니 데스크탑에 320×100이 와서 10px 밀렸다). Ezoic 래퍼 `span.ezoic-ad`의 margin 15px×2는 **인라인 `!important`**라 스타일시트로는 못 이긴다 — `AdCard`의 `stripEzoicMargins`가 같은 인라인 `!important`로 0을 덮어쓴다(관찰자가 style 변경도 보므로 리프레시 후에도 유지) — 소재 위아래 빈 띠의 정체이자 예약에 안 잡히던 36px 시프트의 원인이었다 (#104) 레일(옆)·시트(아래)는 밀릴 컨텐츠가 없어 예약하지 않는다
