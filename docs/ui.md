...
- **✅ 이 둘은 정상 동작이다 — 버그로 다시 파지 말 것** (2026-09-01 실측)
  ...
  - **상세 시트를 열면 기존 광고가 ~2초 비었다가 4자리 전부 새로 채워진다** → 바로 위 "배치에는 살아 있는 자리를 전부 넣는다" 규칙의 **의도된 비용**이다. 실측 타임라인: +1.0s 전부 빔 → +1.5s 예약 높이 → +3.0s 전부 복구(`#103` 신규 포함). 대안은 `show(103)` 단독인데 그러면 레일이 **영구 사망**한다(#94). 게다가 어차피 30초마다 리프레시되는 자리라 그 주기를 앞당긴 것에 가깝다
