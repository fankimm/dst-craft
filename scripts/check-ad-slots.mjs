...
// 시트 안에는 광고 자리가 없다 (#110에서 103 제거). 열어 둔 채 기다려도 배치가 나가면
// 안 된다 — 시트가 상단 띠(111)의 주인을 바꾸거나 레일을 건드리면 여기서 드러난다.
await step("시트 유지", null, 3000);
...
for (const r of results) {
  ...
  // 시트에는 자리가 없으므로(#110) 열기·유지·닫기 어느 단계에서도 배치가 나가면 안 된다 —
  // 나갔다면 시트가 살아 있는 자리의 주인을 갈아 끼운 것이고, 그 배치마다 4자리가 비워진다
  if (/^(아이템 클릭\(상세 시트\)|시트 유지|시트 닫기)$/.test(r.step) && r.calls.length) {
    problems.push(`${r.step}: 시트에 자리가 없는데 배치가 나감 — 시트가 다른 자리의 주인을 바꾸고 있다 [${r.calls.join(" ")}]`);
  }
