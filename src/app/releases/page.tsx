...
const releases: Release[] = [
  ...
  },
  {
    ...
    date: "2026-09-01",
    dev: [
      "fix(ads): 상세 시트 자리에 **안정화 대기**를 두어 광고 배치 폭주를 없앴다 (#96). `#94`(배치에 살아 있는 자리를 전부 넣기)의 부작용이다 — 시트는 열고 닫을 때마다 자리가 생겼다 사라지므로 배치가 쏟아졌다. 프로덕션 실측: 아이템 5개를 각 4초씩 열고 닫자 **배치 15건, 레일이 0.9~3.1초 간격으로 9번 리프레시**. Ezoic 자체 리프레시 주기가 30초인데 그 15배다.",
      "왜 손해인가 — 뷰어빌리티 집계 기준이 **1초**(픽셀 50% 이상)라 0.9초 만에 갈아치운 노출은 집계도 지불도 되지 않으면서, 평균 viewability 를 끌어내려 **전체 인벤토리의 CPM** 을 깎는다(현재 display viewability 69.47%, 3개 포맷 중 최저 — `docs/ezoic-decision.md` 기준선). 초 단위 반복 리프레시는 invalid traffic 정책 리스크이기도 하다.",
      "**\"배치에서 레일을 빼면 되지 않나\"는 안 된다는 것을 실측으로 재확인**했다. 시트가 열린 상태에서 `showAds(103)` 만 단독 호출하니 레일 `#107`·`#108` 이 즉시 비워지고 6초 뒤에도 복구되지 않았다. 그래서 고칠 지점은 \"배치에 무엇을 넣느냐\"가 아니라 **\"배치를 몇 번 내느냐\"** 였다.",
      "`claim: 2000ms` — 시트가 이만큼 떠 있어야 자리로 인정한다. 스쳐 지나가는 상세뷰는 등록조차 하지 않는다. **잃는 것이 없다**: Ezoic이 자리를 채우는 데 4~7초가 걸리므로 2초 미만 열람에서는 요청했어도 못 채우고 닫힌다. `linger: 4000ms` — 닫혀도 이만큼 붙들고 있어, 아이템을 연달아 훑는 동안 자리가 등록된 채로 남아 배치가 아예 안 나간다(다시 열리면 해제 예약 취소). 상단 띠·레일은 화면에 상주하므로 대기 0 — 첫 광고가 늦어지면 안 된다.",
      "beta 실측 (같은 시나리오): 배치 **15건 → 5건**, 최소 간격 **0.9초 → 4.0초**, gampad 요청 **45건 → 16건**, `destroyPlaceholders` 호출은 중간에서 사라짐(열기와 합쳐짐). 마지막 닫기 뒤에는 linger 만료 시점에 `destroy(103)` + 나머지 재요청이 정상 발화 — 사라진 자리에 리프레시가 남지 않는다.",
      "`scripts/check-ad-slots.mjs` 에 \"시트 유지(임계 초과)\" 단계와 검사를 추가했다. `claim` 지연이 \"시트 광고가 영영 안 뜬다\"로 조용히 바뀌는 것을 막는다. `FLUSH_DEBOUNCE_MS`(250)는 건드리지 않았다 — `docs/ezoic-decision.md` 의 손대면 안 되는 항목.",
    ],
    changes: { ko: [], en: [] },
  },
  {
  ...
  },
  {
    ...
    date: "2026-09-01",
    dev: [
      "fix(ads): 광고 배치를 낼 때 **그 시점에 살아 있는 자리를 전부 함께 요청**하도록 수정 (#94). 상세패널을 열면 사이드 레일 `#107`·`#108` 이 높이 0인 빈 div가 되고 **시트를 닫아도 돌아오지 않았다** — 그 세션 동안 레일은 영구 사망이고, 레일은 최근 7일 수익의 13%다(`docs/ezoic-decision.md`). 탭 전환에서도 같은 일이 일어나고 있었다.",
      "원인은 `#75`가 남긴 틀린 전제였다. \"지목하지 않은 자리까지 비워지는\" 현상을 **`destroy` 전용**으로 결론내고 `flushAdQueue` 가 해제가 낀 배치에서만 나머지를 재요청하게 했는데, **`showAds` 단독으로도 똑같이 비운다**. 실제 호출을 가로채 보니 상세 시트를 열 때 나가는 배치가 `showAds(111)` 하나뿐이었고(`#111` 의 주인이 바뀌어서) 그 호출에 레일 둘이 죽었다. destroy가 없으니 복구 로직도 안 탔다.",
      "수정 후 배치: 상세 시트 `show(103)` → **`show(103,107,108,111)`**, 탭 전환 `show(111)` → **`show(111,107,108)`**. 화면 전환(`카테고리 → 목록`)은 `#93` 효과로 여전히 배치가 나가지 않는다. 배치 자체는 `shownOwner` 비교로 변화가 있을 때만 나가므로 유휴 상태의 반복 요청은 없다.",
      "`scripts/check-ad-slots.mjs` 에 \"배치에서 빠진 자리\" 검사 추가 — 목업으로도 우리 쪽 불변식은 잡히게 했다. 음성 대조로 수정 전 프로덕션에서 8건 검출(탭 전환 7 + 상세 시트 1), 수정본 0건. beta 실측에서 상세패널을 열어도 레일이 7초 내 복구되고 시트를 닫아도 유지됨을 확인.",
      "docs/ui.md 의 틀린 전제 정정 + 회귀 확인법. docs/mistakes.md 2건 — API 동작을 일부 호출 형태로만 확인하고 일반화한 것, 그리고 배포 직후 실측이 옛 번들을 잡아 같은 세션에서 두 번 오진한 것(결과가 수정 전과 *완전히 동일*하면 코드가 아니라 배포를 먼저 의심할 것).",
    ],
    changes: {
      ko: [
        "아이템 상세를 열면 좌우 광고가 사라진 뒤 다시 돌아오지 않던 문제를 고쳤습니다.",
      ],
      en: [
        "Fixed the side ads disappearing for good after opening an item's detail view.",
      ],
    },
  },
  {
    ...
    date: "2026-09-01",
    dev: [
      "fix(ads): 제작 탭 상단 광고 자리(`#111`)를 화면별 컴포넌트 밖으로 올려 **화면 전환 시 재요청을 제거** (#93). `AdSlot variant=\"top\"` 이 `CategoryGrid` 와 `ItemGrid` **안에** 각각 있었다 — 두 컴포넌트는 화면 전환 때 서로 교체되므로 placeholder를 쥔 DOM 노드가 통째로 바뀌고, Ezoic은 새 div를 발견해 처음부터 다시 요청했다. 프로덕션 실측: 홈에서 채워진 광고가 **카테고리를 누르는 순간 빈칸(iframe 0)이 되고**(재요청 @14,717ms) 8초 뒤에야 다시 찼다. 전환할 때마다 사용자는 빈 띠를 4~8초 다시 봤고 노출 시간도 매번 0으로 초기화됐다 — 실사용자 계측의 `early`(소재 도착 전 이탈) 56%와 직결된다.",
      "`CraftingApp` 이 홈/목록 두 개의 `return` 으로 갈려 있던 것을 **하나로 합쳤다**. 헤더·본문만 삼항으로 갈리고 `AdSlot` 은 트리에 하나만 존재한다 — 같은 위치·같은 타입이라 React가 DOM 노드를 재사용하고 Ezoic 입장에서 자리가 사라진 적이 없다. 두 `return` 에 각각 자리를 넣어 위치가 우연히 맞아떨어지길 기대하는 대신 통합하는 쪽을 골랐다(그 방식은 앞에 엘리먼트 하나만 끼어들어도 조용히 깨진다).",
      "beta 실측으로 확인: 카테고리 클릭 직후에도 `#111` 이 `iframe 1` 로 유지되고 `Content_1` 재요청이 발생하지 않는다(전: 요청 2건 → 후: 1건). 부수적으로 `pushState` 자체는 광고를 리셋하지 않고, 슬라이드 애니메이션도 무관함을 각각 분리 실험으로 확인했다.",
      "레이아웃 회귀 없음 — 광고 자리 높이 143px 동일, 검색바→첫 카드 간격은 오히려 줄었다(데스크탑 182→170px, 모바일 174→166px). `scripts/check-ad-cls.mjs` 띠 계열 시프트 전부 0, `scripts/check-ad-slots.mjs` problems 0이며 \"카테고리 → 목록\"·\"홈으로\" 단계의 `calls` 가 빈 배열(전에는 각각 `show(111)`).",
      "docs/ui.md 에 **\"번호를 쥔 노드는 화면 전환에도 살아 있어야 한다\"** 규칙과 회귀 확인법 추가. 요리(3곳)·보스(2곳)·스킬(2곳)·스킨·퀘스트·콘솔은 같은 구조가 남아 있어 후속 확산 대상 — SEO 정적 페이지는 페이지 이동이 곧 새 문서라 해당 없음.",
    ],
    changes: {
      ko: [
        "카테고리를 눌러 화면을 옮길 때 상단 영역이 깜빡였다가 다시 채워지던 현상을 고쳤습니다.",
      ],
      en: [
        "Fixed the top area flickering and reloading when you switch screens by tapping a category.",
      ],
    },
  },
  {
  ...
  },
  {
    ...
    date: "2026-08-27",
    dev: [
      "fix(ads): 광고 도달 계측(#85)의 판정을 **고정 4초 대기 → 도착 관측**으로 바꿨다 (#86). 4초 체크포인트는 실제 첫 크리에이티브(데스크탑 콜드 4.1~5.4초, 상단 띠 111은 4.9~5.9초, Fast3G 15~17초, 사용자 눈대중 6~7초)보다 일러, **광고가 오는 중인 시점에 판정해 그 표본이 통째로 `nofill`로 집계**됐다. ePMV 저하 원인을 재고 부족과 단가로 가르려던 지표가 그 판정을 스스로 오염시키고 있었다. 상수의 근거가 \"AdSlot 배치 창 1.5초 + 입찰·렌더\"라는 추정이었고 종단 시간을 재지 않은 것이 원인.",
      "250ms 폴링으로 소재 도착을 관측해 붙는 즉시 `filled` 보고 — 6초가 걸리든 17초가 걸리든 오판이 없다. 고정 시각(20초)은 \"끝내 안 오는 경우\"의 데드라인으로만 남긴다. `MutationObserver`는 쓰지 않았다: 놓치지 않으려면 `document.body` subtree를 봐야 하는데 그러면 앱 리렌더마다 콜백이 돌아 데드라인까지 프레임마다 강제 리플로우가 걸린다(느림을 재려다 앱을 느리게 만드는 꼴). 폴링은 250ms×20초 = 최대 80회로 비용 상한이 있고, 자리별 세밀한 감시는 `AdCard`가 이미 한다.",
      "첫 소재 도착 시각(ms)을 함께 보고 → 서버가 구간 버킷(`0-2s`~`20s+`)으로 집계하고 `/stats` 광고 도달 섹션에 실사용자 지연 분포를 표시. 지연 조사가 전부 headless에서 나왔고 Ezoic이 클라이언트를 분류하는 정황이 있어 절대값을 실사용자로 확증할 방법이 없었다 — 이 분포가 그 확증이다. 도착 시각은 `filled` 세션만 집계한다(no-fill의 값은 \"데드라인까지 기다린 시간\"이라 섞으면 20초 구간만 부풀린다).",
      "검증: beta 실측에서 소재 주입 6550ms → 비콘 6712ms 발화(`filled:true`), 데드라인 경로 `elapsed:21261`(`filled:false`), longTask 0건, CLS 0.0019 무변. 실제 UA로 재측정 시 3개 자리(107·111·108) 전부 충전되며 `filled:true, elapsed:7177` — 사용자 눈대중 6~7초와 일치하고, 옛 4초 기준이었다면 이 세션이 `nofill`로 집계됐을 것.",
      "docs: 광고 렌더 지연 원인 조사 결과를 `todo.md`에 기록. **첫 광고까지 약 5초 중 4초(80%)가 `window load` 이후 Ezoic·Google 파이프라인**이고, Ezoic이 `window load`를 기다린다는 것을 인과로 확정(대역폭을 안 쓰는 1×1 gif로 load만 +2379.5ms 밀자 `sacountry.go`도 +2379.4ms, Δ≤0.3ms, 4/4). 입찰 타임아웃은 페이지뷰마다 500/750/875/2000ms 중 랜덤(`gampad = requestBids + T + 24ms`).",
      "docs: **손대면 안 되는 항목 7종**을 실측 근거와 함께 남겼다 — `s.async=false` 직렬화(페널티 중앙값 0ms, `async=true`는 오히려 +34ms), `FLUSH_DEBOUNCE_MS=250`(슬랙 680~777ms에 숨음), CMP 제거·지역분기, gpt.js preload·preconnect, 하이드레이션 단축, 자동 자리 116·117 끄기, `ezoicanalytics.js` 제거(차단하면 광고가 아예 안 뜨는 하드 의존성). 되돌리면 #75/#79 회귀를 부르는 것들이라 다시 파지 않도록 기록.",
      "docs/mistakes.md 오답노트 3건: 서로 다른 기간의 두 숫자를 나눠 visits 격차를 15배로 오진(실제 1.2배), 타이밍 상수를 실측 없이 추정으로 확정, 헤드리스에서 광고가 안 뜨는 걸 \"수익화 대상 아님\"으로 오독(Ezoic이 UA로 클라이언트를 분류).",
    ],
    changes: { ko: [], en: [] },
  },
  {
