...
export function SettingsPage() {
  ...
  const [newFeedbackItem, setNewFeedbackItem] = useState<PublicFeedbackItem | null>(null);
  ...
  const handleFeedback = useCallback(async () => {
    ...
    if (id) {
      ...
      setNewFeedbackItem({ id, message: msg, time: new Date().toISOString(), status: "new", reply: null });
    ...
    <div className="flex flex-col h-full bg-background text-foreground overflow-y-auto overscroll-contain">
      <div className="flex flex-col min-h-full">
        <div className="flex-1 p-4 space-y-6 max-w-md mx-auto w-full">
          ...
          {/* Feedback board (public + admin in one) */}
          <FeedbackBoard locale={resolvedLocale} newItem={newFeedbackItem} />
