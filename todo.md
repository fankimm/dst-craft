...
> ~~다음 최우선: 주소확인 PIN 대응~~ → **2026-08-31 인증 완료로 종료.** 이제 최우선은 3-1 수익 개선 레버(anchor / 측정 격차) → 10/15 Ramp-up 판단.
>
> 📄 **결정 문서: `docs/ezoic-decision.md`** — 10/15 판단은 이 문서 하나로 끝나게 정리해 뒀다. 판단 기준(숫자), 데이터 읽는 법, 분기별 실행 절차.
> 세션 시작 시 `scripts/check-deadlines.sh`가 D-30부터 자동 경고한다.
>
> **기억할 날짜 2개: 🔴 `10/15` 계속 여부 판단(통보 기한) · `11/11` Ramp-up 만료(넘기면 1년 자동 확정)**
> ✅ ~~`10/2` 주소확인 PIN 마감~~ — **2026-08-31 인증 완료로 해소.** 광고 게재 중단 리스크가 사라졌으므로 10/15 판단의 근거인 EPMV 데이터는 그때까지 끊기지 않고 쌓인다.

#### 1. 돈이 실제로 들어오게 하는 것 — ✅ **2026-08-31 전부 완료** (마지막까지 남아 있던 주소 확인 PIN 인증 통과)
...
- [x] **Ad Manager 도메인 심사 ✅ 통과 (2026-08-20 확인)** — 8/17 제출 → `Approved`
  ...
  - ⚠️ setup 화면의 *"Ad demand and revenue are limited until Ad Manager is fully set up"* 경고는 **주소 확인(Pending) 하나 때문**이었다
    - → 2026-08-31 주소 확인 완료. **Ezoic 표시는 항상 늦으므로** 며칠 지켜볼 것. 9월 첫째 주까지도 경고가 안 풀리면 그건 별개 원인이므로 따로 확인한다

- [x] **✅ 주소 확인 PIN — 2026-08-31 인증 완료** (8/18 발송 → 8/31 도착·입력. 마감 10/2보다 32일 빠름)
  ...
  - **일정**
    ...
    | ~2026-09-08 | 도착 예상 (구글 안내 "통상 3주"). 안 오면 이 시점부터 재발송 요청 가능이었음 — **실제로는 그 전에 도착해 미발동** |
    | **2026-08-31** | **✅ 우편 도착 → PIN 입력 → 인증 완료.** 실소요 13일로 안내(3주)보다 빨랐다 |
    | ~~2026-10-02~~ | ~~45일 마감. 미인증 시 광고 게재 중단~~ — 32일 남기고 해소 |
  ...
  - [x] **PIN 입력 완료 (2026-08-31)** — Ad Manager `#payments/verification`
    - 결과: 화면의 두 항목이 **`본인 인증: 완료됨` / `주소 확인: 완료됨`** 으로 전환. 1회 입력에 통과 (시도 3회 중 1회 사용)
    - `PIN 다시 보내기`는 누르지 않았다 — 재발송 창(~9/8)이 열리기 전에 우편이 도착해 해당 없음
  - 💸 **지급 보류 해제** — 인증 미완료로 걸려 있던 *"인증 절차가 완료되지 않아 지급이 보류 중입니다"* 경고의 전제 조건이 해소됐다
    - 단 실제 지급은 최소 지급액 $20을 넘겨야 나간다 (현재 월 $13 수준이라 아직 미달)
  - **연쇄가 끊겼다**: 입력 지연 → 광고 중단 → EPMV 데이터 공백 → 10/15 판단 불가로 이어질 뻔한 경로가 사라졌다. 10/15 판단은 온전한 9월 데이터로 할 수 있다
...
- ⚠️ **(2026-09-08 재검증) 아래 §2/§4 인용은 공개 약관에서 확인되지 않았다.** 실물은 대시보드 `Account Details → AGREEMENT → Download`에 있다. 받아서 대조할 것.
- **독점(§2)**: 사이트의 모든 프로그래매틱 인벤토리를 Ezoic이 독점. 다른 네트워크/AdSense 직접 연동 불가, ~~ads.txt도 Ezoic 것만~~ ← **틀렸다.** Ezoic 자사 ads.txt KB가 *"add any others (like your AdSense ID)"* 라고 안내한다. 다만 **"non-Ezoic ad code 제거"** 요구는 Ezoic이 실제로 공개하고 있으므로 유효하다
  - → **AdSense 홈의 `애드센스에 사이트를 연결하세요 → 시작하기` 누르지 말 것.** MCM엔 불필요하고 §2와 충돌
    - ⚠️ **(2026-09-08) 이 금지는 "광고 코드 삽입"에만 적용된다.** `<meta name="google-adsense-account">` 확인 태그는 광고를 게재하지 않아 해당하지 않는다 → #100에서 배포 완료. **사이트 목록에서 `검토 요청`을 누르는 것은 여전히 소유자 몫**
...
#### 3-2. 광고 렌더 지연 — 원인 규명 완료 (2026-08-27 실측 조사, 13개 에이전트)
> **결론: 첫 광고까지 약 5초 중 4초(80%)가 window load 이후 Ezoic·Google 파이프라인이다.** 우리 몫은 20%이고 그마저 광고 코드가 아니라 "페이지가 window load를 언제 띄우느냐"뿐이다.
> 실측: 데스크탑 콜드 `/item` 첫 광고 픽셀 4.1~5.4초(상단 띠 111은 4.9~5.9초), Fast3G 모바일 16.7초, 사용자 눈대중 6~7초. 세션 간 표류 ±1.2초.

- **핵심 발견 — Ezoic은 `window load`를 기다린다 (인과 확정)**
  - `sacountry.go` 요청이 `loadEventEnd` ±3ms에 붙음: 독립 세션 3개에서 24/24, 26/26, 12/12 관측
  - 대역폭을 안 쓰는 1×1 gif를 3초 늦게 응답시켜 `load`만 +2379.5ms 밀자 `sacountry.go`도 +2379.4ms (Δ≤0.3ms, 4/4). 다른 자원 완료 시각은 불변
  - 전달률 0.62 — load 1ms 단축이 광고 0.62ms 단축으로만 온다. 하류가 2.5~3.5초 바닥을 깐다 (웜 캐시로 load를 553ms까지 당겨도 첫 광고는 4.4초로 콜드와 동일)
- **입찰 타임아웃이 페이지뷰마다 랜덤** — `gampad/ads` = `pbjs.requestBids + T + 24ms` (잔차 +16~199ms, n=13). T 분포: 500ms 8회 / 750ms 12회 / 875ms 1회 / **2000ms 4회**(n=25). 회귀식 `t0→페인트 ≈ 2460ms + 1.2×T`
- 🔴 **손대면 안 되는 것 (실측 회수 0ms — 다시 파지 말 것)**
  - `s.async=false` 순차 체인: 직렬화 페널티 **중앙값 0ms**(홈 6회 중 5회 정확히 0). `async=true`는 오히려 +34ms. 체인 종료(~520ms)가 방출 게이트인 `window load`(~1150ms)보다 600ms 일러 구조적으로 회수 불가. 되돌리면 #79 회귀
  - `FLUSH_DEBOUNCE_MS=250`: 우리 push와 Ezoic 큐 드레인 사이 슬랙이 680~777ms라 통째로 숨는다. 낮추면 #75 재발(자리 누락 = 수익 손실)
  - CMP 제거·지역분기 / gpt.js preload·preconnect / 하이드레이션 단축 / 116·117 끄기 / 인터스티셜 끄기 / CLS 작업 — 전부 회수 없음 또는 수익 손실
...
- **할 만한 것 (기대효과 순)**
  ...
  - [ ] Ezoic 지원 티켓: 입찰 타임아웃 T 상한 하향 요청 (실측 근거 첨부). **초안 완료 → `docs/ezoic-ticket-bid-timeout.md` (#124, 2026-09-21). 9/22 판독 뒤에 제출 — 그 전에 보내면 측정 구간 오염.** 파트너 `insticator`(57건 중 낙찰 0, 타임아웃 24회, 응답 중앙값 1375ms)·`yieldmo`(응답 0, 타임아웃 7회) 검토 요청 — 단 낙찰 0은 헤드리스·데이터센터 IP 편향 가능성이 있어 **타임아웃 하향을 먼저**
  - [ ] 홈 초기 페이로드 지연 로딩 (269KB 청크, RSC prefetch, GSI, `/api/{rating,top-countries,feedback/public,popular}` 4건)
- ~~**미해결**~~ **해소 (2026-09-21, #124)**: 실사용자 첫 광고 도착 중앙값 **약 7.0초**(n=3,957, 최빈 6-8s 38.5%) — headless(약 5초)보다 2초 느리다. 8/31 판독(n=675)과 분포 모양이 동일해 #91~#97 경량화는 도착 시각을 못 당겼다. 9/14~9/20 판정값 70.7%, 광고를 본 세션 20.1%. 지연 의심 7항목 재점검 표와 함께 `docs/ezoic-decision.md` 2026-09-21 절에 기록. (원문: 이번 조사는 전부 headless에서 나왔고, 기본 headless UA로는 우리 포함 Ezoic 계열 5개 사이트가 **전부 0% 충전**됐다(UA만 바꾸면 충전). Ezoic이 클라이언트를 분류한다는 뜻이라 절대값이 실사용자와 다를 수 있다 → **#86의 실사용자 도착 시각 분포가 이 의심을 푼다**)
- **미해결 2**: 한 배치에서 8런 연속 `showAds` 미호출·`gampad` 0건이 관측됐다(직후 동일 조건은 정상). 계측 버그인지 Ezoic의 무광고 A/B·봇 판정인지 미확인. 후자라면 체감 원인이 "지연"이 아니라 **"간헐적 미표시"** 이고 별개 문제다
...
- [x] **농사 탭 1~3단계** (피드백 2026-08-29 "추천 비료/작물 조합") — 2026-09-17 #120. 파이프라인 + 계절별 조합 추천 + 작물 상세 + 참고표 + `/farming` 정적 페이지. 남은 것(4단계 밭 배치도 등)은 `TODO-farming-tab.md` 6장
