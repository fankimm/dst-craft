...
- [~] **🔴 [진행중] 주소 확인 PIN — 2026-08-18 발송됨. 시계 돌기 시작. 마감 2026-10-02**
  ...
  - **[ ] 지금 할 일: 수취인 주소 점검** (Ad Manager → 결제 → 결제 정보 → 설정 관리 → 비즈니스 이름과 주소 ✎)
    - 구글이 명시한 조건: **모든 입력란을 동일 언어로** 작성 / 현지 우체국이 인식 가능한 형식
    - 지급 프로필 예금주명은 로마자 강제였으므로(위 참조) **우편 수취인명 표기가 배달 가능한지** 별도 확인
    - 주소가 틀리면 3주 허비 + 재발송 3주 → **10/2를 넘긴다.** 지금 고치는 것이 유일한 예방책
