...
### UI/UX
- [ ] 스킬 노드 카드 가로폭 고정 (현재 텍스트 길이에 따라 가변)
- [ ] 브레드크럼 다른 탭(제작/요리/보스)과 통일성 있게 스타일 맞추기
- [ ] 스킬 상세 패널에 해금 조건(부모 스킬, 잠금 게이트 조건) 표시
- [ ] 습득 포인트 표현 변경 — "습득 0/23" 대신 인게임 최대 스킬 포인트 확인 후 "남은 포인트: N" 형식

### 데이터/번역
- [ ] 번역 검수 — ko.po 한글모드 파일 기준으로 전체 스킬 번역 크로스체크
- [ ] 인게임 캐릭터별 최대 스킬 포인트 확인 (tuning.lua 또는 skilltreedata.lua)

### 잠금 로직
- [ ] lockType 없는 잠금 노드 처리 — 위그프리드 전투영창통(인벤 6개 악보), 워톡스 성향 등 커스텀 조건
- [ ] 시뮬레이터에서 해당 조건 우회 방법 결정 (수동 토글? 항상 해금?)
